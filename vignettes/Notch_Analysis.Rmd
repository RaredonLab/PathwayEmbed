...
```{r}
# Extract Matrix data
COI_matrix <- as.matrix(GetAssayData(cluster_of_interest, assay = "RNA", slot = "data"))
COI_metadata <- cluster_of_interest@meta.data

# Compute using PathwayEmbed 
COI_notch <- ComputeCellData(COI_matrix, "Notch", "manhattan", batch.size = 1000, scale.data = TRUE)
To_plot <- PreparePlotData(COI_metadata, COI_notch, "Age")
